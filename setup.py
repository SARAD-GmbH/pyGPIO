...
def manual_board_assignment():
    print(
        ...
        + print_red("This is only recommended for experienced users! ")
        + "\nPin mapping might be "
        + print_red("false")
        + " and the library "
        + print_red("does not work")
        + " as expected!"
    )
    ...
    for line in cpuinfo:
        if "Hardware" in line:
            ...
            if "sun4i" in processor:
                print(
                    "Detected processor: "
                    + print_yellow(processor)
                    + " (Probably Allwinner A10)"
                )
    ...
    for line in boardinfo:
        if "BOARD" in line:
            ...
            if "orangepizero" == board:
                ...
                print_correct()
                shutil.copy2(
                    "pyGPIO/gpio/mapping/orangepizero.h", "pyGPIO/gpio/mapping.h"
                )
                ...
                print_correct()
                shutil.copy2(
                    "pyGPIO/gpio/mapping/orangepipcplus.h", "pyGPIO/gpio/mapping.h"
                )
                ...
                print_correct()
                shutil.copy2(
                    "pyGPIO/gpio/mapping/orangepiplus2e.h", "pyGPIO/gpio/mapping.h"
                )
                ...
                print_correct()
                shutil.copy2(
                    "pyGPIO/gpio/mapping/orangepilite.h", "pyGPIO/gpio/mapping.h"
                )
            ...
            # FriendlyArm Boards
                ...
                print_correct()
                var6 = input(
                    "Do you want to use your NanoPi Duo with [1] ore without minishield [2]\n(mapping & naming in connector mode is incorrect, no changes by using port)?)"
                )
                if var6 == "2" or var6 == "[2]":
                    shutil.copy2(
                        "pyGPIO/gpio/mapping/nanopiduo.h", "pyGPIO/gpio/mapping.h"
                    )
                elif var6 == "1" or var6 == "[1]":
                    shutil.copy2(
                        "pyGPIO/gpio/mapping/nanopiduo_minishield.h",
                        "pyGPIO/gpio/mapping.h",
                    )
                else:
                    print("Abort.")
                    sys.exit(1)
            ...
            if "bananapim2" == board:
                ...
                print_correct()
                shutil.copy2(
                    "pyGPIO/gpio/mapping/bananapim2.h", "pyGPIO/gpio/mapping.h"
                )
...
modules = [
    Extension(
        "pyGPIO.gpio.gpio", sources=["pyGPIO/gpio/gpio_lib.c", "pyGPIO/gpio/gpio.c"]
    ),
